...
class KimiRecipeAgent:
    ...
    def extract_recipe_info(self, markdown_content: str, file_path: str = "") -> RecipeInfo:
        ...
        # 构建提示词
        category_hint = (
            f"，根据文件路径推断此菜谱属于【{inferred_category}】分类" if inferred_category else ""
        )
        category_placeholder = (
            inferred_category
            or '菜谱分类（素菜/荤菜/水产/早餐/主食/汤类/甜品/饮料/调料，支持多个分类用逗号分隔，如"早餐,素菜"）'
        )
...
"""
        ...
        messages = [
            {
                "role": "system",
                "content": "你是一个专业的菜谱分析专家，擅长从中文菜谱中提取结构化信息。",
            },
    ...
    def _fallback_parse(self, content: str) -> RecipeInfo:
        ...
        return RecipeInfo(name=name or "未知菜谱", difficulty=difficulty, category=category)
...
class RecipeKnowledgeGraphBuilder:
    ...
    def __init__(
        self, ai_agent: KimiRecipeAgent, output_dir: str = "./ai_output", batch_size: int = 20
    ):
    ...
    def save_progress(
        self, current_file: str = None, total_files: int = 0, processed_count: int = 0
    ):
        """保存处理进度"""
        progress_data = {
    ...
    def merge_all_batches(self):
        ...
        if all_concepts:
            ...
            final_concepts.to_csv(
                os.path.join(self.output_dir, "concepts.csv"), index=False, encoding="utf-8"
            )
        ...
        if all_relationships:
            ...
            final_relationships.to_csv(
                os.path.join(self.output_dir, "relationships.csv"), index=False, encoding="utf-8"
            )
            print(f"合并关系: {len(final_relationships)} 个")

        return len(final_concepts) if all_concepts else 0, len(
            final_relationships
        ) if all_relationships else 0
    ...
    def _generate_recipe_synonyms(self, name: str, category: str) -> List[str]:
        ...
        if name.endswith("的做法"):
            ...
            synonyms.extend([f"{base_name}制作方法", f"{base_name}烹饪方法", base_name])
    ...
    def _categorize_synonyms_by_language(self, synonyms: List[str]) -> List[dict]:
        ...
        for synonym in synonyms:
            ...
            if self._is_english(synonym):
                categorized.append({"term": synonym, "language": "en", "language_code": "en-US"})
            elif self._is_chinese(synonym):
                categorized.append({"term": synonym, "language": "zh", "language_code": "zh-CN"})
            else:
                # 默认为中文
                categorized.append({"term": synonym, "language": "zh", "language_code": "zh-CN"})
    ...
    def batch_process_recipes(self, recipe_dir: str, resume: bool = True) -> Tuple[int, int]:
        ...
        try:
            ...
            print(
                f"处理完成: 成功 {processed_count - already_processed} 个，失败 {failed_count} 个"
            )
    ...
    def export_to_csv(self, output_dir: str):
        ...
        # 导出概念
        concepts_df = pd.DataFrame(self.concepts)
        concepts_df.to_csv(os.path.join(output_dir, "concepts.csv"), index=False, encoding="utf-8")

        # 导出关系
        relationships_df = pd.DataFrame(self.relationships)
        relationships_df.to_csv(
            os.path.join(output_dir, "relationships.csv"), index=False, encoding="utf-8"
        )
    ...
    def export_to_rf2_format(self, output_dir: str):
        ...
        with open(os.path.join(output_dir, "rf2_description.txt"), "w", encoding="utf-8") as f:
            ...
            for concept in all_concepts:
                ...
                if "fsn" in concept and concept["fsn"]:
                    f.write(
                        f"D{desc_id_counter:06d}\t20241201\t1\t900000000\t{concept_id}\t"
        ...
        with open(os.path.join(output_dir, "rf2_relationship.txt"), "w", encoding="utf-8") as f:
            ...
            for relationship in self.relationships:
                f.write(
                    f"R{rel_id_counter:06d}\t20241201\t1\t900000000\t"
        ...
        print("- rf2_description.txt: 包含首选术语和同义词")
    ...
    def export_to_neo4j_csv(self, output_dir: str, merge_batches: bool = True):
        ...
        nodes_df.to_csv(os.path.join(output_dir, "nodes.csv"), index=False, encoding="utf-8")
...
def main():
    ...
    parser.add_argument(
        "--format", choices=["csv", "neo4j"], default="neo4j", help="输出格式 (csv 或 neo4j)"
    )
    parser.add_argument("--base-url", default="https://api.moonshot.cn/v1", help="Kimi API基础URL")
