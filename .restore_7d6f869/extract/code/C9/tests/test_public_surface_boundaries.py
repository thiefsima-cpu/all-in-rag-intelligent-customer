...
class PublicSurfaceBoundaryTests(unittest.TestCase):
    ...
    def _resolve_import_from(cls, path: Path, node: ast.ImportFrom) -> str:
        ...
        return resolve_name(relative_name, cls._module_name_for_path(path))
    ...
    def test_scripts_do_not_import_default_config_from_repo_root_facade(self) -> None:
        violations: list[str] = []
        ...
        for path in (ROOT / "scripts").rglob("*.py"):
            ...
            for node in ast.walk(tree):
                if isinstance(node, ast.ImportFrom) and node.module == "config":
                    ...
                    if "DEFAULT_CONFIG" in imported_names:
                        violations.append(f"{rel}:{node.lineno}: {lines[node.lineno - 1].strip()}")

        self.assertFalse(
            violations,
            "Found scripts importing DEFAULT_CONFIG from the repo-root config facade:\n"
            + "\n".join(violations),
        )
    ...
    def test_retirement_plan_document_exists(self) -> None:
        plan_path = ROOT / "docs" / "public_surface_retirement_plan.md"
        self.assertTrue(plan_path.exists())
        content = plan_path.read_text(encoding="utf-8")
        self.assertIn("Root Facade Mapping", content)
        self.assertIn("Retirement Phases", content)
    ...
    def test_system_runtime_and_legacy_access_use_facade_support(self) -> None:
        ...
        found_interactive_service_call = False
            ...
            if isinstance(node, ast.ImportFrom):
                ...
                if func_name in {"resolve_grouped_legacy_attribute", "merge_legacy_dir_names"}:
                    ...
                    if chain == "self.interactive_service.run_interactive":
                        found_interactive_service_call = True
        ...
        self.assertTrue(
            found_interactive_service_call,
            "system interactive entrypoint should delegate through interactive_service",
        )
        self.assertFalse(
            violations,
    ...
    def test_system_interactive_service_owns_cli_orchestration(self) -> None:
        path = RAG_MODULES_DIR / "app" / "composition" / "system_interactive_service.py"
        rel = path.relative_to(ROOT)
        source = path.read_text(encoding="utf-8-sig")
        tree = ast.parse(source, filename=str(path))
        lines = source.splitlines()
        found_operations_access = False
        found_cli_console_import = False
        violations: list[str] = []

        for node in ast.walk(tree):
            if isinstance(node, ast.ImportFrom):
                module_name = self._resolve_import_from(path, node)
                if module_name.endswith("interfaces.cli_console"):
                    found_cli_console_import = True
            elif isinstance(node, ast.Import):
                for alias in node.names:
                    if alias.name.endswith("interfaces.cli_console"):
                        found_cli_console_import = True
            elif isinstance(node, ast.Attribute):
                chain = ".".join(self._attribute_chain(node))
                if chain.startswith("self.operations_service."):
                    found_operations_access = True
                if chain.startswith("self.runtime_manager.") or chain.startswith("self.facade_support."):
                    violations.append(f"{rel}:{node.lineno}: {lines[node.lineno - 1].strip()}")

        self.assertTrue(
            found_cli_console_import,
            "system interactive service should own the CLI console dependency",
        )
        self.assertTrue(
            found_operations_access,
            "system interactive service should enforce readiness/close through operations_service",
        )
        self.assertFalse(
            violations,
            "Found interactive-service orchestration that should not depend on runtime_manager/facade_support:\n"
            + "\n".join(violations),
        )
        ...
        for node in ast.walk(class_node):
            if isinstance(node, ast.Call) and isinstance(node.func, ast.Name):
                if node.func.id in prohibited_calls:
                    ...
                    if "interactive_service" not in keyword_names:
                        violations.append(f"{rel}:{node.lineno}: missing interactive_service=")
    ...
    def test_application_service_composer_wires_facade_support(self) -> None:
        ...
        found_interactive_call = False
        found_interactive_operations = False
        violations: list[str] = []
        ...
        for node in ast.walk(class_node):
            if isinstance(node, ast.Call) and isinstance(node.func, ast.Name):
                ...
                elif node.func.id == "SystemInteractiveService":
                    found_interactive_call = True
                    keyword_names = {kw.arg for kw in node.keywords if kw.arg is not None}
                    found_interactive_operations = "operations_service" in keyword_names
                    if "operations_service" not in keyword_names:
                        violations.append(f"{rel}:{node.lineno}: missing interactive operations_service=")
        ...
        self.assertTrue(
            found_interactive_call,
            "application service composer should construct SystemInteractiveService",
        )
        self.assertTrue(
        ...
        self.assertTrue(
            found_interactive_operations,
            "application service composer should wire operations_service into SystemInteractiveService",
        )
        self.assertTrue(
