...
class EntityLinker:
    ...
    def __init__(self, driver: Optional[Driver], database: str = "neo4j", config=None):
        self.driver = driver
        self.database = database
        self.config = config
        self.limit_per_entity = int(getattr(config, "entity_linker_limit_per_entity", 4) if config else 4)
        self.min_confidence = float(getattr(config, "entity_linker_min_confidence", 0.45) if config else 0.45)
        self.max_same_name_candidates = int(
            getattr(config, "entity_linker_max_same_name_candidates", 2) if config else 2
        )
        ...
        self.query_type_label_priorities = dict(
            getattr(config, "entity_linker_query_type_label_priorities", {}) if config else {}
        )
        self.relation_label_priorities = dict(
            getattr(config, "entity_linker_relation_label_priorities", {}) if config else {}
        )
