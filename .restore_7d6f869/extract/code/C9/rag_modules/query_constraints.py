...
def parse_minutes(value: Any) -> Optional[int]:
    ...
    total = 0.0
    hour_matches = re.findall(r"(\d+(?:\.\d+)?)\s*(?:小时|小時|h|hr|hour)", text, flags=re.I)
    minute_matches = re.findall(r"(\d+(?:\.\d+)?)\s*(?:分钟|分鐘|min|minute)", text, flags=re.I)
...
class QueryConstraintExtractor:
    ...
    def extract(self, query: str) -> QueryConstraints:
        from .query_semantics import infer_query_semantic_profile

        profile = infer_query_semantic_profile(query)
...
class RecipeConstraintMatcher:
    ...
    @staticmethod
    def _recipe_minutes(doc: Document) -> Tuple[Optional[int], Optional[int], Optional[int]]:
        metadata = doc.metadata or {}
    ...
    def score(self, doc: Document, constraints: QueryConstraints) -> Tuple[bool, float, List[str]]:
        ...
        if self._contains_any(text, constraints.exclude_terms + constraints.excluded_ingredients):
            return False, 0.0, ["命中排除词"]
        if constraints.excluded_cuisine_terms and self._contains_any(cuisine, constraints.excluded_cuisine_terms):
            return False, 0.0, ["命中排除菜系"]
        if constraints.max_prep_minutes is not None and prep is not None and prep > constraints.max_prep_minutes:
            return False, 0.0, ["准备时间超限"]
        if constraints.max_cook_minutes is not None and cook is not None and cook > constraints.max_cook_minutes:
            return False, 0.0, ["烹饪时间超限"]
        if constraints.max_total_minutes is not None and total is not None and total > constraints.max_total_minutes:
        ...
        if constraints.cuisine_terms and self._contains_any(cuisine, constraints.cuisine_terms):
            score += 1.0
        if constraints.category_terms and self._contains_any(category, constraints.category_terms):
            score += 1.0
        if constraints.max_total_minutes is not None:
            if total is not None:
                score += 2.0
                reasons.append(f"时间约{total}分钟")
            else:
                score -= 0.5
    ...
    def filter_and_rank(
        ...
        for doc in self.documents:
            ...
            metadata["search_type"] = metadata.get("search_type", "constraint_recipe")
            scored.append(Document(page_content=doc.page_content, metadata=metadata))

        scored.sort(key=lambda d: d.metadata.get("constraint_score", 0.0), reverse=True)
        return scored[:limit]
