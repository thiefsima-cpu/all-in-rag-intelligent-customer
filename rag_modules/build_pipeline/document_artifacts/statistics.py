...
class DocumentArtifactStatsCollector:
    ...
    def collect(self, data_module) -> DocumentArtifactStats:
        ...
        return DocumentArtifactStats(
            total_recipes=int(
                stats.get("total_recipes", len(getattr(data_module, "recipes", []) or []))
            ),
            ...
            total_cooking_steps=int(
                stats.get(
                    "total_cooking_steps", len(getattr(data_module, "cooking_steps", []) or [])
                )
            ),
            total_documents=int(
                stats.get("total_documents", len(getattr(data_module, "documents", []) or []))
            ),
            total_chunks=int(
                stats.get("total_chunks", len(getattr(data_module, "chunks", []) or []))
            ),
        )
